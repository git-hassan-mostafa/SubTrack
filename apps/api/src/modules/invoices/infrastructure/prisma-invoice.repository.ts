...
import { Invoice, InvoiceStatus } from '@subtrack/shared';
...
export class PrismaInvoiceRepository implements InvoiceRepository {
  ...
  async create(data: Omit<Invoice, 'id' | 'createdAt' | 'updatedAt' | 'paidDate'> & { id?: string }): Promise<Invoice> {
    const totalInvoices = await this.prisma.invoice.count({ where: { tenantId: data.tenantId } });
    const invoiceNumber = data.invoiceNumber || `INV-${new Date().getFullYear()}-${String(totalInvoices + 1).padStart(4, '0')}`;
  ...
  async update(id: string, data: Partial<Omit<Invoice, 'id' | 'tenantId' | 'createdAt' | 'updatedAt'>>): Promise<Invoice> {
