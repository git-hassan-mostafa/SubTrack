...
PLAIN ENGLISH DESCRIPTION:
Now we build the login system. Each generator company (tenant) already exists
in the system (created by a super‑admin or seeded). The tenant’s administrator
can create additional users (operators, collectors, etc.) via a dedicated
“Users” screen in the dashboard. Those users receive their credentials
(email and password) out of band. The mobile app only provides a login
screen – there is no registration flow. Upon login, the backend returns a
JWT token containing the user’s ID, their tenant ID, and their role.
...
- users table: id, tenantId (foreign key), email, passwordHash, name,
  role (enum: TENANT_ADMIN, OPERATOR, MANAGER), createdAt, updatedAt
...
- **User management endpoints (protected by TENANT_ADMIN role):**
  - POST /users – Create a new user (email, name, role, temporary password).
    The password is hashed before storage. Returns the created user (without
    password hash).
  - GET /users – List all users of the authenticated tenant.
  - PATCH /users/:id – Update user (name, role, or password reset).
  - DELETE /users/:id – Deactivate or delete user (soft delete optional).
- **No registration endpoint** – tenants and their first admin are created
  either by a super‑admin via a separate internal tool or via database seeding.
...
- **No registration screen or “Sign Up” button anywhere.**

ACCEPTANCE CRITERIA:

- Logging in with a pre‑created user returns a JWT with userId, tenantId, and role.
...
- A tenant administrator can create, view, and modify users via API endpoints.
- No way to self‑register – the app never shows a registration form.
================================================================================
================================================================================
...
- STATE LAYER (/state):
  Must contain: State management (Zustand),
